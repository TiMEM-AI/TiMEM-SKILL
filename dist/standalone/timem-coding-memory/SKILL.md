...
`S3` overview/module/arch questions are **Must** (not Skip); implement/edit/explain/follow-up → **Should**/`S3`. When unsure → search. Flow: search first → verify → then read code; then `project_discovery` write gate.
